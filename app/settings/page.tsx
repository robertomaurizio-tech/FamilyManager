...
import { uploadCsvData, deleteAllData, getCategorie, saveLoginSequence, getLoginSequence } from '@/lib/actions';
...
export default function SettingsPage() {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>, fileType: 'vacanze' | 'spese') => {
    if (e.target.files && e.target.files[0]) {
      if (fileType === 'vacanze') {
        setVacanzeFile(e.target.files[0]);
      } else {
        setSpeseFile(e.target.files[0]);
      }
      setStatus('idle');
      setMessage('');
    }
  ...
  const handleDrop = (e: React.DragEvent<HTMLDivElement>, fileType: 'vacanze' | 'spese') => {
    ...
    if (e.dataTransfer.files && e.dataTransfer.files[0]) {
      if (fileType === 'vacanze') {
        setVacanzeFile(e.dataTransfer.files[0]);
      } else {
        setSpeseFile(e.dataTransfer.files[0]);
      }
      setStatus('idle');
      setMessage('');
    }
  ...
  const handleUpload = async () => {
    if (!vacanzeFile || !speseFile) {
      setMessage('Seleziona entrambi i file CSV prima di importare.');
      setStatus('error');
      return;
    }
    ...
    const readVacanzeFile = new Promise<string>((resolve, reject) => {
      const reader = new FileReader();
      reader.onload = (event) => resolve(event.target?.result as string);
      reader.onerror = (error) => reject(error);
      reader.readAsText(vacanzeFile);
    });

    const readSpeseFile = new Promise<string>((resolve, reject) => {
      const reader = new FileReader();
      reader.onload = (event) => resolve(event.target?.result as string);
      reader.onerror = (error) => reject(error);
      reader.readAsText(speseFile);
    });

    try {
      const [vacanzeCsv, speseCsv] = await Promise.all([readVacanzeFile, readSpeseFile]);
      const result = await uploadCsvData(vacanzeCsv, speseCsv);
      if (result.success) {
        setStatus('success');
        setMessage(result.message || 'Dati importati con successo!');
        setVacanzeFile(null);
        setSpeseFile(null);
      } else {
        setStatus('error');
        setMessage(result.message || 'Errore durante l\'importazione.');
      }
    } catch (error: any) {
      setStatus('error');
    ...
    <div className="space-y-8">
      ...
      <section className="bg-white p-8 rounded-3xl border border-zinc-100 shadow-sm space-y-6">
        ...
        <button
          ...
          disabled={!vacanzeFile || !speseFile || status === 'uploading'}
